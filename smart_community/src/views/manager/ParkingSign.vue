  ...
  <div>
    ...
    <div class="table">
      <el-table :data="tableData" strip @selection-change="handleSelectionChange">
        ...
        <el-table-column prop="parkingId" label="停车位id"></el-table-column>
        <el-table-column prop="parkingAddress" label="停车位地址"></el-table-column>
    ...
    <el-dialog userName="审核信息" :visible.sync="fromVisible" width="40%"
  ...
  methods: {
    ...
    save() {   // 保存按钮触发的逻辑  它会触发新增或者更新
      this.$refs.formRef.validate((valid) => {
        if (valid) {
          this.$request({
            url: this.form.id ? '/parkingSign/update' : '/parkingSign/add',
    ...
    delBatch() {   // 批量删除
      ...
      this.$confirm('您确定批量删除这些数据吗？', '确认删除', {type: "warning"}).then(() => {
        this.$request.delete('/parkingSign/delete/batch', {data: this.ids}).then(res => {
          if (res.code === '200') {   // 表示操作成功
            this.$message.success('操作成功')
            this.load(1)
          } else {
    ...
    load(pageNum) {  // 分页查询
      ...
      this.$request.get('/parkingSign/selectPage', {
        params: {
