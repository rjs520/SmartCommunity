<template>
  <div class="main-content">
    <div>
      <el-row :gutter="10">
        <el-col :span="6" v-for="item in complaintSuggestionList" :key="item.id">
          <div class="complaintSuggestion-item"
               @click="$router.push('/front/complaintSuggestionDetail?id='+ item.id)">
            ...
            <div
              ...
              <div style="font-size: 12px;color: #666c">{{ item.date }}</div>
            </div>
          </div>
        </el-col>
      </el-row>
...
<script>
export default {
  name: 'FrontComplaintSuggestion',
  data() {
    return {
      complaintSuggestionList: [],
      pageSize: 8,
      pageNum: 1,
      total: 0,
    }
  ...
  methods: {
    load(pageNum) {  // 分页查询
      ...
      this.$request.get('/complaintSuggestion/selectPage', {
        ...
        this.complaintSuggestionList = res.data?.list
