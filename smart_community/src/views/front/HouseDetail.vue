<template>
  <div style="width: 60%; margin: 10px auto">
    <div style="display: flex;grid-gap: 20px">
      ...
      <div style="flex: 1">
        <div style="margin: 20px 0;display: flex;align-items: center">
          ...
          <div style="color: #666;font-size: 13px">{{ nurseHouse.count }}浏览量</div>
        </div>
        <div style="margin: 10px 0">
          <el-tag style="margin-right: 10px" v-for="item in JSON.parse(nurseHouse.tags || '[]')"
                  :key="item">{{ item }}
          </el-tag>
        </div>
        <div style="margin: 10px 0; color: #ffa400; font-size: 18px; font-weight: bold">
          ￥{{ nurseHouse.price }}
        </div>
        <div style="margin: 20px 0; color: #666">{{ nurseHouse.address }}</div>
        <div style="display: flex; margin: 10px 0;">
          <div style="width: 200px"><span style="color: #666">机构性质：</span>{{
              nurseHouse.nature
            }}
          </div>
        ...
        <div style="display: flex; margin: 10px 0;">
          <div style="width: 200px"><span style="color: #666">成立时间：</span>{{ nurseHouse.date }}
          </div>
          <div style="flex: 1"><span style="color: #666">床位数量：</span>{{ nurseHouse.beds }}</div>
        </div>
        <div style="display: flex; margin: 10px 0;">
          <div style="width: 200px"><span style="color: #666">机构面积：</span>{{ nurseHouse.area }}
          </div>
        </div>

        <div style="margin: 20px 0">
          电话：<span style="font-size: 20px; font-weight: bold; color: orangered">{{
            nurseHouse.phone
          }}</span>
        </div>
      </div>
    ...
    <div style="margin: 30px 0">
      <div style="font-weight: bold; font-size: 22px; text-align: center; margin-bottom: 30px">
        详细信息
      </div>
